...
func TestFragmentationEvaluator(t *testing.T) {
	...
	for i := 1; i <= len(points); i++ {
		box := backend.Box{ID: i, Height: 1, Width: 1}
...
func TestFragmentationEvaluatorLarge(t *testing.T) {
	...
	for i, x := range []int{40, 80} {
		wall := backend.Box{ID: i + 1, Height: height, Width: 1}
...
func TestFragmentationEvaluatorEdgeCases(t *testing.T) {
	tests := []struct {
		name  string
		world func(t *testing.T) *backend.World
		...
		},
		{
			...
			world: func(t *testing.T) *backend.World {
				...
				box := backend.Box{ID: 1, Height: 4, Width: 5}
			...
			world: func(t *testing.T) *backend.World {
				...
				for y := 0; y < 3; y++ {
					for x := 0; x < 3; x++ {
						...
						box := backend.Box{ID: boxID, Height: 1, Width: 1}
...
func TestFutureFitProbability(t *testing.T) {
	world := newTestWorld(t, 3, 3)
	if err := world.Container.Place(backend.Box{ID: 1, Width: 1, Height: 1}, 1, 1, false); err != nil {
		t.Fatal(err)
	}

	got := FutureFitProbability(world, backend.UniformBoxDistribution{
		MinWidth:  1,
		MaxWidth:  2,
		MinHeight: 1,
		MaxHeight: 2,
